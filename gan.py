...
def CheckAccuracy( x, g_z, data_cols, label_cols=[], seed=0, with_class=False, data_dim=2 ):
    ...
    '''
    base_n_count = 128
    inputs = layers.Input((dtrain.shape[1]-1,))
    outputs = layers.Dense(base_n_count, activation='relu')(inputs)
    outputs = layers.Dropout(0.3)(outputs)
    outputs = layers.Dense(base_n_count*4, activation='relu')(outputs)
    ...
    #x = layers.Dense(1)(x)
    classify_model = models.Model(inputs=[inputs], outputs=[outputs], name='classification_model')
    ...
    checkpointer = ModelCheckpoint('model-nn-classify.h5', verbose=1, save_best_only=True)
    ...
    results = classify_model.fit(X_train, y_train, validation_split=0.1, batch_size=32, epochs=10,
                        callbacks=[earlystopper, checkpointer])

    model = models.load_model('model-nn-classify.h5', custom_objects={'auc': auc})
...
def classify_network(x, base_n_count):
    x = layers.Dense(base_n_count, activation='relu')(x)
...
def adversarial_training_WGAN(arguments, train, data_cols, label_cols=[], seed=0, starting_step=0):
    ...
    #disc_optimizer = optimizers.Adadelta(lr=1.0, rho=0.95, epsilon=None, decay=0.0)

    sess = K.get_session()

    # compile models

    adam = optimizers.Adadelta(lr=learning_rate, rho=0.95, epsilon=None, decay=0.0)
